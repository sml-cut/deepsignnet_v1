...
def read_video(src, source_reverse):
    ...
    mean_image = np.load('/usr/src/slt/slt/video/FulFrame_Mean_Image_227x227.npy').astype(np.float32)[..., ::-1]
    ...
    # for each image
    for i in range(0, len(images)):
        image_ = cv2.imread((src + images[i]).decode('UTF-8'))
        if image_ is not None:
            video[i, :, :, :] = image_.astype(np.float32) - mean_image
        else:
            print(src + images[i])
            input()
